...
async def create_seed_data():
    ...
    async with async_session_maker() as db:
        ...
        for vote_data in votes_data:
            ...
            result = await db.execute(
                text(
                    "SELECT id FROM options WHERE poll_id = :poll_id AND text = :text"
                ),
            ...
            await db.commit()
            print(
                f"✅ {vote_data['voter']} voted {vote_data['option_text']} on {vote_data['poll_title']}"
            )
        ...
        result = await db.execute(
            text(
                "SELECT id FROM delegations WHERE delegator_id = :delegator_id AND delegate_id = :delegate_id"
            ),
            {
        ...
        if existing_delegation:
            print(
                f"✅ Delegation from {delegation_data['delegator']} to {delegation_data['delegate']} already exists"
            )
        else:
            # Create delegation with custom timestamp
            created_at = datetime.utcnow() - timedelta(
                hours=delegation_data["hours_ago"]
            )
            ...
            await db.commit()
            print(
                f"✅ {delegation_data['delegator']} delegated to {delegation_data['delegate']}"
            )

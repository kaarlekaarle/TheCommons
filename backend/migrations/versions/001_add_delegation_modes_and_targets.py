...
def upgrade() -> None:
    ...
    op.add_column('delegations', sa.Column('mode', sa.String(length=20), nullable=False, server_default='flexible_domain'))
    op.add_column('delegations', sa.Column('is_anonymous', sa.Boolean(), nullable=True, server_default='false'))
    op.add_column('delegations', sa.Column('field_id', postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column('delegations', sa.Column('institution_id', postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column('delegations', sa.Column('legacy_term_ends_at', sa.DateTime(timezone=True), nullable=True))

    # Add foreign key constraints
    op.create_foreign_key(None, 'delegations', 'fields', ['field_id'], ['id'], ondelete='SET NULL')
    op.create_foreign_key(None, 'delegations', 'institutions', ['institution_id'], ['id'], ondelete='SET NULL')

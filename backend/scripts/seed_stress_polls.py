...
async def create_poll_with_labels(
    db, poll_data: dict, user: User, labels: List[Label], created_at: datetime
):
    ...
    if poll_data["decision_type"] == "level_a":
        ...
        options = [
            ...
            Option(
                poll_id=poll.id, text="No", created_at=created_at, updated_at=created_at
            ),
            Option(
                poll_id=poll.id,
                text="Abstain",
    ...
    if labels:
        label_associations = [
            {"poll_id": str(poll.id), "label_id": str(label.id)} for label in labels
        ]
        await db.execute(
...
async def seed_stress_polls():
    ...
    async for db in get_db():
        try:
            ...
            total_associations = await db.execute(
                "SELECT COUNT(*) as count FROM poll_labels"
            )

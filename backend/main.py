...
from backend.core.redis import close_redis_client, get_redis_client
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    response = get_error_response(
        exc, include_details=settings.DEBUG, request_id=request_id
    )
...
async def get_popular_labels_public(
    limit: int = Query(
        8, ge=1, le=24, description="Number of popular labels to return (max 24)"
    ),
    ...
    if not settings.TESTING:
        ...
        latest_label_result = await db.execute(
            ...
            .where(and_(Label.is_active == True, Label.is_deleted == False))
            .order_by(desc(Label.updated_at))
            .limit(1)
        )
...
async def comprehensive_health_check():
    ...
    try:
        ...
        health_status["services"]["rate_limiter"] = {
            "status": (
                "healthy" if limiter_status.get("status") == "healthy" else "unhealthy"
            ),
    ...
    try:
        ...
        health_status["services"]["websocket"] = {
            ...
            "message": (
                "WebSocket manager is healthy"
                if ws_healthy
                else "WebSocket manager is unhealthy"
            ),
...
async def limiter_health_check(user: User = Depends(get_current_user)):
    ...
    from backend.core.admin_audit import log_admin_action, require_admin

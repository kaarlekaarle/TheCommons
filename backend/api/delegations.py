...
@router.post(
    "/", response_model=DelegationResponse, status_code=status.HTTP_201_CREATED
)
async def create_delegation(
    ...
    logger.info(
        ...
        extra={
            "delegation_data": delegation_in.model_dump(),
    ...
    try:
                ...
                if hasattr(delegation_in, "label_slug") and delegation_in.label_slug:
                    ...
                    if not label:
                        raise ValidationError(
                            f"Label with slug '{delegation_in.label_slug}' not found"
                        )
                    label_id = label.id
        ...
        is_high_conc, conc_level, conc_percent = (
            await concentration_monitor.is_high_concentration(
        ...
        super_risk, super_reason, super_stats = (
            await super_delegate_detector.would_create_super_delegate(
                delegation_in.delegatee_id, field_id
            )
        )
        if super_risk:
        ...
        try:
            ...
            context = {
                ...
                "has_warnings": bool(
                    warnings.get("concentration", {}).get("active")
                    or warnings.get("superDelegateRisk", {}).get("active")
                ),
            }
            await telemetry_service.track_delegation_mode(
                current_user.id, mode, context
            )
        ...
        try:
            ...
            logger.warning(
                f"Failed to broadcast delegation creation", extra={"error": str(e)}
            )
        ...
        return {
            "delegation": {
                ...
                "created_at": (
                    delegation.created_at.isoformat() if delegation.created_at else None
                ),
                "updated_at": (
                    delegation.updated_at.isoformat() if delegation.updated_at else None
                ),
            },
            "warnings": warnings,
        }
...
async def get_my_delegation(
    ...
    try:
        ...
        delegate_result = await db.execute(
            select(User).where(
                and_(User.id == delegation.delegatee_id, User.is_deleted == False)
            )
        )
...
async def get_my_delegation_summary(
    ...
    try:
        ...
        if settings.LABELS_ENABLED:
            ...
            for delegation in label_delegations:
                ...
                delegate_result = await db.execute(
                    select(User).where(
                        and_(
                            User.id == delegation.delegatee_id, User.is_deleted == False
                        )
                    )
                )
                delegate = delegate_result.scalar_one_or_none()
        ...
        return DelegationSummary(
            global_delegate=global_delegate_info, per_label=per_label_delegations
        )
...
async def unified_search(
    ...
    types: str = Query(
        "people,fields,institutions", description="Comma-separated list of target types"
    ),
...
async def get_adoption_telemetry(
    ...
    try:
        ...
        mode_counts_query = (
            select(Delegation.mode, func.count(Delegation.id).label("count"))
            .where(
                and_(
                    Delegation.is_deleted == False,
                    Delegation.revoked_at.is_(None),
        ...
        hybrid_users_query = select(
            func.count(func.distinct(Delegation.delegator_id))
        ).where(
            and_(
...
async def get_delegatee_inbound_delegations(
    ...
    field_id: Optional[UUID] = Query(
        None, description="Optional field ID to filter by"
    ),
    ...
    try:
        ...
        # Get delegatee info
        delegatee_result = await db.execute(select(User).where(User.id == delegatee_id))
        delegatee = delegatee_result.scalar_one_or_none()
        ...
        for delegation in delegations:
            ...
            inbound_item = {
                ...
                "expiresAt": (
                    delegation.end_date.isoformat() if delegation.end_date else None
                ),
                "legacyTermEndsAt": (
                    delegation.legacy_term_ends_at.isoformat()
                    if delegation.legacy_term_ends_at
        ...
        for field_id, count in top_fields:
            if field_id == "global":
                top_fields_with_names.append(
                    {"fieldId": field_id, "fieldName": "Global", "count": count}
                )
            else:
                field_name = field_names.get(field_id, field_id)
                top_fields_with_names.append(
                    {"fieldId": field_id, "fieldName": field_name, "count": count}
                )
        ...
        return {
            ...
            "delegateeName": delegatee.username,
            "inbound": inbound,
            "counts": {"total": total_count, "byField": field_counts},
            "summary": {
...
async def get_delegation_health_summary(
    limit: int = Query(
        10, ge=1, le=50, description="Maximum number of top delegatees to return"
    ),
    current_user: User = Security(get_current_active_user),
    db: AsyncSession = Depends(get_db),
) -> dict:
    ...
    try:
        ...
        top_delegatees_query = (
            select(Delegation.delegatee_id, func.count(Delegation.id).label("count"))
            .where(
                and_(
                    Delegation.is_deleted == False,
                    Delegation.revoked_at.is_(None),
            ...
            .order_by(func.count(Delegation.id).desc())
            .limit(limit)
        )
        ...
        by_field_query = (
            ...
            .group_by(Delegation.field_id, Delegation.delegatee_id)
            .order_by(Delegation.field_id, func.count(Delegation.id).desc())
        )

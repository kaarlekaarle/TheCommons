...
class SafeDelegationSummaryService:
    ...
    async def get_safe_summary(self, user_id: Optional[str], trace_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            global_delegate_info = await self._get_global_delegation_safe(user_id, errors)
            
            # Get per-label delegations safely
            per_label_delegations = await self._get_label_delegations_safe(user_id, errors)
    ...
    async def _get_global_delegation_safe(self, user_id: str, errors: List[str]) -> Optional[Dict[str, Any]]:
        """Safely get global delegation info."""
        try:
            global_delegation = await self.delegation_service.get_active_delegation(user_id)
            ...
            return {
                ...
                "created_at": global_delegation.created_at.isoformat() if global_delegation.created_at else None
            }
    ...
    async def _get_label_delegations_safe(self, user_id: str, errors: List[str]) -> List[Dict[str, Any]]:
            ...
            for delegation in label_delegations:
                try:
                    ...
                    logger.warning(
                        ...
                        extra={"user_id": user_id, "delegation_id": delegation.id, "error": str(e)}
                    )

...
class ChainResolutionCore:
    ...
    def resolve_chain_from_delegations(
        ...
        delegation_map = ChainResolutionCore._build_delegation_map(
        ...
        while depth < max_depth:
            ...
            if ChainResolutionCore._has_user_override_placeholder(
                current_user_id,
                poll_id,
                label_id,
                field_id,
    ...
    def _find_active_delegation(
        ...
        for delegation in user_delegations:
            if ChainResolutionCore._is_delegation_active_or_expired_legacy(
                delegation, now
            ):
    ...
    @staticmethod
    def _is_delegation_active_or_expired_legacy(
        delegation: Delegation, now: datetime
    ) -> bool:
    ...
    def _is_delegation_active(delegation: Delegation, now: datetime) -> bool:
        ...
        if (
            delegation.is_legacy_fixed_term
            and delegation.legacy_term_ends_at
            and now > delegation.legacy_term_ends_at
        ):
            return False
    ...
    def _matches_target_scope(
        ...
        if poll_id is not None:
            ...
            return (
                delegation.poll_id is None
                and delegation.label_id is None
                and delegation.field_id is None
                and delegation.institution_id is None
                and delegation.value_id is None
                and delegation.idea_id is None
            )
    ...
    def deserialize_chain(chain_data: List[Dict[str, Any]]) -> List[Delegation]:
        ...
        for delegation_data in chain_data:
            ...
            delegation.poll_id = (
                UUID(delegation_data["poll_id"]) if delegation_data["poll_id"] else None
            )
            delegation.label_id = (
                UUID(delegation_data["label_id"])
                if delegation_data["label_id"]
                else None
            )
            delegation.field_id = (
                UUID(delegation_data["field_id"])
                if delegation_data["field_id"]
                else None
            )
            delegation.institution_id = (
                UUID(delegation_data["institution_id"])
                if delegation_data["institution_id"]
                else None
            )
            delegation.value_id = (
                UUID(delegation_data["value_id"])
                if delegation_data["value_id"]
                else None
            )
            delegation.idea_id = (
                UUID(delegation_data["idea_id"]) if delegation_data["idea_id"] else None
            )
            delegation.start_date = (
                datetime.fromisoformat(delegation_data["start_date"])
                if delegation_data["start_date"]
                else None
            )
            delegation.end_date = (
                datetime.fromisoformat(delegation_data["end_date"])
                if delegation_data["end_date"]
                else None
            )
            delegation.legacy_term_ends_at = (
                datetime.fromisoformat(delegation_data["legacy_term_ends_at"])
                if delegation_data["legacy_term_ends_at"]
                else None
            )
            delegation.created_at = (
                datetime.fromisoformat(delegation_data["created_at"])
                if delegation_data["created_at"]
                else None
            )
            chain.append(delegation)
        return chain

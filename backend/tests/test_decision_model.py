...
@pytest.mark.asyncio
async def test_poll_create_level_a_requires_choice(db_session: AsyncSession, test_user):
    ...
    response = client.post("/api/polls/", json=poll_data, headers={"Authorization": f"Bearer {test_user.token}"})
...
@pytest.mark.asyncio
async def test_poll_create_level_a_missing_choice_fails(db_session: AsyncSession, test_user):
...
@pytest.mark.asyncio
async def test_poll_create_level_b_ignores_direction_choice(db_session: AsyncSession, test_user):
...
@pytest.mark.asyncio
async def test_poll_read_includes_new_fields(db_session: AsyncSession, test_user):
    ...
    create_response = client.post("/api/polls/", json=poll_data, headers={"Authorization": f"Bearer {test_user.token}"})
    assert create_response.status_code == 200
    
    poll_id = create_response.json()["id"]
    
    # Read the poll
    read_response = client.get(f"/api/polls/{poll_id}", headers={"Authorization": f"Bearer {test_user.token}"})
...
@pytest.mark.asyncio
async def test_level_a_blocked_when_feature_disabled(db_session: AsyncSession, test_user, monkeypatch):
...
@pytest.mark.asyncio
async def test_level_b_works_when_feature_disabled(db_session: AsyncSession, test_user, monkeypatch):
...
@pytest.mark.asyncio
async def test_poll_list_includes_new_fields(db_session: AsyncSession, test_user):
    ...
    client.post("/api/polls/", json=poll_data, headers={"Authorization": f"Bearer {test_user.token}"})
    
    # Get poll list
    response = client.get("/api/polls/", headers={"Authorization": f"Bearer {test_user.token}"})
    assert response.status_code == 200

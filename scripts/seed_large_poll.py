...
async def seed_large_poll():
    """Seed a large poll with direct votes and delegations."""
    print("🚀 Starting large poll seeding...")
    ...
    async with async_session_maker() as session:
        ...
        poll, options = await create_test_poll_with_options(
            session,
            creator,
            title="Large Performance Test Poll",
            description="A poll to test performance with 1000 users (500 direct + 500 delegated)",
        ...
        # Create 500 direct voters
        print("📝 Creating 500 direct voters...")
        direct_voters = []
        for i in range(500):
            user = await create_test_user(
                session,
                f"direct_voter_{i:03d}",
                f"direct_voter_{i:03d}@test.com",
                f"password_{i:03d}"
            )
            ...
            if (i + 1) % 50 == 0:
                print(f"   Created {i + 1}/500 direct voters")
        
        print(f"✅ Created {len(direct_voters)} direct voters")
        
        # Create 500 users who delegate their votes
        print("🔗 Creating 500 users with delegations...")
        delegation_users = []
        ...
        for i in range(500):
            ...
            if (i + 1) % 50 == 0:
                print(f"   Created {i + 1}/500 delegations")
        ...
        return poll.id, total_users, len(direct_votes), len(delegations)


async def measure_poll_results_performance(poll_id: UUID):
...
async def main():
    ...
    print("🎯 Large Poll Seeding and Performance Test")
    print("=" * 50)
    
    try:
        # Seed the large poll
        poll_id, total_users, direct_votes, delegations = await seed_large_poll()
        ...
        # Measure performance
        response_time = await measure_poll_results_performance(poll_id)

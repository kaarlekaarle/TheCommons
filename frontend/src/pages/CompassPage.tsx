...
import { ArrowLeft, Share2, FileText, Calendar, Clock, CheckCircle } from 'lucide-react';
...
export default function CompassPage({ scheduler = defaultScheduler }: CompassPageProps) {
  ...
  const handleVoteSubmit = async (optionId: string) => {
    ...
    try {
      ...
      setVoteError(error.message || 'Failed to align with this direction');
      compassAnalytics.error('vote', error.message || 'Failed to cast vote');
    } finally {
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="space-y-4">
          ...
          <p className="text-gray-700 text-lg leading-relaxed">{compassCopy.framing}</p>
        </div>
        ...
        {myAlignment && (
          <div 
            ...
            data-testid="alignment-banner"
            aria-live="polite"
            aria-label="Alignment confirmation"
          >
            <div className="flex items-center gap-3">
              <CheckCircle className="w-5 h-5 text-green-600" />
              <div className="flex-1">
                <p className="text-green-900 font-medium">
                  {compassCopy.alignedWith} <strong>{options.find(o => o.id === myAlignment)?.text}</strong>. 
                  You can change your alignment any time.
                </p>
              </div>
              <Button 
                variant="ghost" 
        ...
        {/* Direction Options */}
        <section>
          <h2 className="text-xl font-semibold text-gray-900 mb-6">{compassCopy.directionsTitle}</h2>
        ...
        {/* Tally */}
        <section>
          <h2 className="text-xl font-semibold text-gray-900 mb-6">{compassCopy.tallyTitle}</h2>
        ...
        <section>
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl font-semibold text-gray-900">{compassCopy.contextTitle}</h2>
          </div>
          <div className="p-6 bg-white rounded-xl border border-gray-200">
            <p className="text-gray-700 mb-4">
              This compass establishes foundational principles that will guide future community decisions. 
              The chosen direction will influence how we approach related topics and proposals.
            </p>
